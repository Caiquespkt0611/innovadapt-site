...
export default function Metodo() {
  return (
    <section id="metodo" className="secao border-t border-white/[0.06]">
      <div className="wrap">
        <Revelar>
        ...
        <div className="mt-14 grid gap-px overflow-hidden rounded-2xl border border-white/[0.08] bg-white/[0.07] md:grid-cols-2">
          {metodo.map((m, i) => (
            <Revelar key={m.numero} delay={(i % 2) * 70} className="h-full">
              <div className="flex h-full flex-col bg-[#080b13] p-7 md:p-9">
                ...
                <p className="mt-3 flex-1 text-sm leading-relaxed text-[#98a4bb]">
                  {m.texto}
                </p>
                <p className="mt-6 border-l-2 border-[#00b4ff] pl-4 text-sm font-semibold text-white">
                  {m.marcador}
                </p>
              </div>
            </Revelar>
